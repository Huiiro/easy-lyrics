...
let currentLocale: 'zh-CN' | 'en-US' = Intl.DateTimeFormat().resolvedOptions().locale.toLowerCase().startsWith('zh')
  ? 'zh-CN'
  : 'en-US'

const mainEnglish: Record<string, string> = {
  '导出歌词': 'Export Lyrics', '文件': 'File', '音频文件': 'Audio Files', '所有文件': 'All Files',
  '选择音频': 'Select Audio', '保存工程': 'Save Project', '打开工程': 'Open Project',
  '未保存的修改': 'Unsaved Changes', '当前工程有未保存的修改。': 'The current project has unsaved changes.',
  '自动恢复副本已保留，但建议先保存工程。': 'An autosave is available, but saving the project is recommended.',
  '取消关闭': 'Cancel', '仍然关闭': 'Close Anyway', '新建': 'New', '打开项目…': 'Open Project…',
  '最近项目': 'Recent Projects', '暂无最近项目': 'No Recent Projects', '重命名项目…': 'Rename Project…',
  '关闭项目': 'Close Project', '保存': 'Save', '查看保存记录': 'View Save History', '导入歌曲…': 'Import Audio…',
  '导入歌词…': 'Import Lyrics…', '导出…': 'Export…', '编辑': 'Edit', '撤销': 'Undo', '重做': 'Redo',
  '播放与打轴': 'Playback & Timing', '播放 / 暂停': 'Play / Pause', '记录当前 Token': 'Mark Current Token',
  '智能打轴': 'Auto Timing', '切换 Loop': 'Toggle Loop', '切换歌词跟随': 'Toggle Lyrics Follow',
  '切换时间轴跟随': 'Toggle Timeline Follow', '导航与微调': 'Navigation & Nudge', '上一个 Token': 'Previous Token',
  '下一个 Token': 'Next Token', '上一句': 'Previous Line', '下一句': 'Next Line', '定位选中 Token': 'Locate Selected Token',
  '向前微调 1 ms': 'Nudge Earlier 1 ms', '向后微调 1 ms': 'Nudge Later 1 ms',
  '向前微调 50 ms': 'Nudge Earlier 50 ms', '向后微调 50 ms': 'Nudge Later 50 ms',
  '复制与 Token 结构': 'Copy & Token Structure', '复制整句时间': 'Copy Line Timing', '粘贴整句时间': 'Paste Line Timing',
  '拆分 Token': 'Split Token', '合并前一个 Token': 'Merge Previous Token', '合并下一个 Token': 'Merge Next Token',
  '时间轴': 'Timeline', 'Token 编辑模式': 'Token Edit Mode', '整句编辑模式': 'Line Edit Mode',
  '切换相邻锁定': 'Toggle Adjacent Lock', '缩小时间轴': 'Zoom Out', '放大时间轴': 'Zoom In',
  '适合时间轴': 'Fit Timeline', '快捷键与设置…': 'Shortcuts & Settings…', '文件后缀': 'Files'
}
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.loadExportTemplates, async () => {
    try {
      ...
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') console.warn('Unable to read export presets', error)
  ...
  ipcMain.handle(
    ...
    async (_event, content: string, extension: string, projectName: string) => {
      ...
      const result = await dialog.showSaveDialog({
        ...
        filters: [{ name: `${safeExtension.toUpperCase()} ${mt('文件后缀')}`, extensions: [safeExtension] }]
      })
...
function createWindow(): void {
  ...
  mainWindow.on('close', (event) => {
    ...
    const choice = dialog.showMessageBoxSync(mainWindow, {
      type: 'warning',
      title: mt('未保存的修改'),
      message: mt('当前工程有未保存的修改。'),
      detail: mt('自动恢复副本已保留，但建议先保存工程。'),
      buttons: [mt('取消关闭'), mt('仍然关闭')],
      defaultId: 0,
      cancelId: 0
    })
